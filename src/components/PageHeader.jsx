...
export default function PageHeader({ eyebrow, title, intro }) {
  return (
    <section className="relative overflow-hidden border-b border-white/10 px-4 pt-36 pb-16 sm:px-6 md:px-12 md:pt-44 md:pb-24">
      <div
        aria-hidden="true"
        className="bg-brand/15 pointer-events-none absolute -top-40 left-1/2 h-96 w-[46rem] -translate-x-1/2 rounded-full blur-3xl"
      />
      <div className="relative mx-auto max-w-7xl">
        <Reveal immediate>
          {eyebrow && (
            <p className="text-brand text-xs font-medium tracking-[0.2em] uppercase">{eyebrow}</p>
          )}
          <h1
